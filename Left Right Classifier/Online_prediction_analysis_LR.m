...
% randomizing all the rows of X, disable this section acc2EE ~= accuracy
% variable
...
for u = 1:size(tmp1EE,1) %replace YEE_sv with tmp1EE if using predicted labels, or replace tmp1EE with YEE_sv for true labels
    if tmp1EE(u)==0 %replace YEE_sv with tmp1EE if using predicted labels, or replace tmp1EE with YEE_sv for true labels
        YEEstring{u}='Left';
    else
        YEEstring{u}='Right';
    end
...
%% SVM Plot left and Right Prediction
